...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  """
    ...
    cur_bs = 0
    while cur_bs < batch_size:
        c = 0
        for i in range(1, skip_window + 1):
            batch[cur_bs] = data[data_index]
            labels[cur_bs][0] = data[data_index - i]
            cur_bs = cur_bs + 1
            c = c + 1
    ...
    # for data_idx in range(start_idx, len(data) - skip_window):
    #     i = 0
    #     for target in range(1, skip_window + 1):
    #         if i == num_skips or cur_bs == batch_size:
    #             break
    #         batch[cur_bs] = data[data_idx]
    #         labels[cur_bs] = data[data_idx - target]
    #         cur_bs = cur_bs + 1
    #         i += 1
    #
    #         if i == num_skips or cur_bs == batch_size:
...
if __name__ == '__main__':
    ...
    with tf.Session(graph=graph) as sess:
        ...
        filename = loss_model
        ex = Workbook()
        sheet = ex.add_sheet('top_words_'+loss_model)
        ...
        for i, word in enumerate(words):
            ...
            sheet.write(0, i, word)
            for k in xrange(top_k):
                close_word = reverse_dictionary[nearest[k]]
                log_str = "%s %s," % (log_str, close_word)
                sheet.write(k + 1, i, close_word)
            print(log_str)
        ex.save(filename+'.xlsx')
